...
class BugForm(FlaskForm):
    ...
    product = StringField(u'归属产品', validators=[Required(), Length(3, 64)])
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.handler.choices = [(user.id, '%s' % user.name if user.name is not None else user.name) for user in User.query.filter_by(disable=False).order_by(User.name.desc()).all()]
